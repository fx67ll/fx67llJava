# MySQL 分区表 + Java 定时任务 实战：基于若依架构的AI日志系统优化
...
### 1.1 什么是月份分区表？# 实战：MySQL分区表+Java定时任务优化AI请求日志系统

在AI应用的开发与运维中，`fx67ll_ai_request_log` 这类请求日志表会随着调用量激增快速膨胀，导致查询性能下降、维护成本上升。本文将结合 **若依（SpringBoot+MySQL）** 架构，详细讲解如何通过 **按月份分区表** 和 **定时任务预统计** 两大方案解决这些痛点，并实现分区的自动化管理。


## 一、优化背景与核心价值
### 1. 问题分析
假设 `fx67ll_ai_request_log` 日增10万条日志，半年即可达1800万条。此时直接查询该表会面临：
- **查询慢**：按时间范围统计需扫描全表索引，响应时间长达数秒；
- **维护难**：删除历史日志需 `DELETE` 全表，锁表且耗时；
- **资源耗**：高并发下实时统计抢占数据库资源，影响主业务。

### 2. 解决方案核心价值
- **分区表**：将大表按月份拆分为物理小文件，查询仅扫描目标分区，效率提升10倍+；
- **定时任务统计**：离线预计算日/月/年数据，前端直接查轻量统计表，响应<100ms；
- **自动化分区管理**：避免人工操作失误，实现分区创建、删除全生命周期自动化。


## 二、优化1：`fx67ll_ai_request_log` 按月份分区
### 1. 分区表原理
MySQL分区表在逻辑上是一张表，物理存储上按 `request_time` 拆分为多个分区文件。查询时自动定位到目标分区，类似“按月份将日志放入不同抽屉”。

### 2. 具体实现步骤
#### （1）前提条件
- MySQL ≥ 5.5（推荐5.7+/8.0+），引擎为 `InnoDB`；
- 分区字段 `request_time` 为 `datetime`/`timestamp` 类型，且需包含在主键中。
...
CREATE TABLE `fx67ll_ai_request_log` (
  ...
  `model_vendor` varchar(30) NOT NULL,
  ...
  `call_status` char(2) DEFAULT '00',
  `error_msg` text,
  `caller_ip` varchar(32) DEFAULT '',
  `request_time` datetime DEFAULT CURRENT_TIMESTAMP,
  `create_by` varchar(64) DEFAULT '',
  PRIMARY KEY (`log_id`, `request_time`) -- 分区字段需包含在主键中
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 
PARTITION BY RANGE (TO_DAYS(request_time)) (
  PARTITION p202602 VALUES LESS THAN (TO_DAYS('2026-03-01')),
  PARTITION p202603 VALUES LESS THAN (TO_DAYS('2026-04-01')),
  PARTITION p_default VALUES LESS THAN MAXVALUE -- 默认分区兜底
);
...
## 三、优化2：定时任务生成日/月/年统计数据
### 1. 核心逻辑
- **日统计**：每日凌晨统计前一天日志，写入 `fx67ll_ai_request_daily_log`；
- **月统计**：每月1日统计上月日数据，写入 `fx67ll_ai_request_monthly_log`；
- **年统计**：每年1月1日统计上年月数据，写入 `fx67ll_ai_request_yearly_log`。
...
## 四、进阶：分区自动化创建与维护
生产环境中严禁手动创建分区，需实现自动化。推荐 **SpringBoot定时任务方案**（贴合若依架构）。

### 1. 核心思路
- 每月25号凌晨3点提前创建下月分区；
- 检查分区是否存在，避免重复创建；
- 保留默认分区兜底，同时实现历史分区自动删除。
...
public class AiRequestStatTask {
    ...
    @Scheduled(cron = "0 0 3 25 * ?") // 每月25号凌晨3点执行
    public void autoCreateNextMonthPartition() {
        try {
            ...
            String partitionLimitDate = nextNextMonth.format(DateTimeFormatter.ISO_DATE);

            // 幂等性检查
            if (aiRequestLogMapper.checkPartitionExists(tableName, partitionName) > 0) {
                log.info("分区{}已存在，无需创建", partitionName);
                return;
            }

            // 执行分区创建：删默认→建下月→重建默认
            aiRequestLogMapper.executePartitionSql("ALTER TABLE " + tableName + " DROP PARTITION p_default");
            String createSql = String.format("ALTER TABLE %s ADD PARTITION (PARTITION %s VALUES LESS THAN (TO_DAYS('%s')))",
                    tableName, partitionName, partitionLimitDate);
            ...
            log.error("分区创建失败", e);
            // 对接告警：alarmService.sendAlarm("分区创建失败", e.getMessage());
        }
    }
...
### 1. 核心价值回顾
- **分区表**：解决海量日志查询慢、维护难问题；
- **定时统计**：将实时计算转为离线预计算，提升前端响应速度；
- **自动化分区**：避免人工失误，实现全生命周期管理。

### 2. 落地优先级
1. 先实现**按月份分区表**，解决日志查询痛点；
2. 再实现**日统计定时任务**，支撑核心报表；
3. 最后扩展**月/年统计**和**分区自动化管理**，满足长期需求。

通过以上方案，可高效支撑AI请求日志的存储、查询与维护，为业务发展提供稳定的数据基础。
